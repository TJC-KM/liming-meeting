...
| Worker | 所有後端邏輯（單檔 ~3,300 行）| `worker/src/index.js` |
...
cron 觸發 runDailyProcess(env, { allowRetry })
   ├─ ⓪ detectStuckPages()   ← 卡死救援，每輪都做（見 3.2）
   ├─ ① KV 佇列（每輪 1 份）  ← 人按的「加入排隊」最優先
   ├─ ② retryOnePending()   ← 只在 06:30、16:00 兩輪（RETRY_CRONS）
   │     有做重試 → 本輪結束（一次 cron 只做一件重活）
   ├─ ③ 自動掃描 Drive 未處理錄音（每輪 1 份，今天上傳的優先，其餘依日期新到舊）
...
①③ 都走 processWithPlaceholder(env, fileId)：
   ...
   4. 失敗 → markAfterFailure() → 狀態=待重試 1/3（檔案過大／空檔 → 直接「失敗」）
```

**為什麼重試只在兩個時段**：以前重試排在每輪最前面，待重試一積多（2026-09 一次積了 54 筆），新上傳的主日講道就要等好幾週。依 Drive 實際上傳時間，週六上午的錄音在 12:30 那輪出現、下午的在 16:00、週二／週五晚上的在 21:30，06:30 那輪幾乎沒有新檔 —— 所以 06:30、16:00 先重試，12:30、21:30 新檔優先，週六兩場都還能當天轉完。改 `wrangler.toml` 的 crons 時 `RETRY_CRONS` 要一起改。`/admin/run-daily` 手動觸發時一律可以重試。

### 哪些錄音不自動轉（只顯示）

`autoSkipReason()` 決定，網站照常顯示、可以播放，行事曆徽章是「🎙 錄音（不自動轉錄）」：

| 原因 | 規則 | 為什麼 |
|---|---|---|
| 非講道錄音 | 檔名含 `AUTO_SKIP_KEYWORDS`（§4.4）| 婚禮、詩班練習不需要 AI 整理重點 |
| 檔案過大 | > `max_auto_size_mb`（預設 150）| Worker 記憶體 128MB。成功過最大的是 135.7MB，160MB 就卡死在「AI 分析中（後半）」。2023 年有一批 400～1100MB 的錄音，自動轉只會連續失敗 3 次、白佔 cron 名額 |
| 檔案過小 | < `min_auto_size_mb`（預設 1）| 不到一分鐘，轉出來幾乎是空的（2026-09-11 一份 0.4MB 的重複上傳就是這樣）|

手動按「加入排隊」會照轉，但 `processAudio` 碰到超過上限或 0 byte 的檔會丟 `permanent` 錯誤 → **直接標「失敗」**並寫明原因（請壓縮成較小的 mp3 再上傳），不進重試。

### cron 列 Drive 一定要翻頁

`listAllAudioCandidates()` 以前只拿第一頁（`pageSize=500`）。2026-09 Drive 有 1,396 個音檔，依檔名倒序排在 500 名以後的（大約 2024-07 以前）**cron 永遠看不到**，沒有任何錯誤訊息。現在每頁 1000 個、最多翻 5 頁（`DRIVE_LIST_MAX_PAGES`），超過會在 log 喊出來。
...
```
detectStuckPages(env)  ← 每次 cron 最先執行
  偵測卡死
...
retryOnePending(env)  ← 只在 RETRY_CRONS 時段（06:30、16:00）
  ① findNextRetryPage：「待重試 1/3」「待重試 2/3」各查一次，挑聚會日期最新的
     （新的講道失敗時不該排在一長串舊錄音後面；同一份最多 3 次就轉「失敗」，不會一直插隊）
  ...
  ② 重跑
     ...
     - 成功 → 草稿
     - 失敗 → markAfterFailure(attempts+1) → 待重試 2/3 → 3 次滿 → 永久「失敗」
```

> ⚠️ **每個重試狀態一定要分開查，不能用一個 `or` 查詢。** Notion 對「select 選項不存在」的過濾會整個回 400：
> `select option "待重試 2/3" not found for property "狀態"`。
> 而「待重試 2/3」這個選項要等第一次重試失敗、`markForRetry` 寫進去時才會被 Notion 自動建立 —— 合在一起查就是死結：查詢永遠失敗 → 永遠沒有重試 → 永遠建不出 2/3。
> 2026-06-27 上線到 2026-09-18 一直是這樣，**54 筆卡在「待重試 1/3」兩個多月一次都沒被重試過**；錯誤只寫在 `console.warn`，而 cron 照常往下處理新檔，所以從外面看不出來。「失敗」選項同理（到 2026-09 還沒有任何一筆用過）。在 Notion 整理「狀態」選項時，**不要刪掉暫時沒用到的選項**。

**手動重試（會議頁的「🔁 重試」）也走排隊**：前端呼叫 `/drive/queue`，cron 的 KV 步驟用 `processQueuedFile()` —— 這個檔只有「待重試／失敗」紀錄時，就地重跑那一筆（`rerunPage`，重試次數歸零），不另建新紀錄。

> ⚠️ 2026-09-18 以前手動重試也從來沒成功過：前端呼叫的 `api.process` 根本不存在（`process` 在 `gasApi` 上），2026-06-26 加上按鈕後一按就報錯；而它原本要走的 `/drive/process` 是在 fetch 回應送出後的 `waitUntil` 裡轉檔，約 30 秒就被砍（實際轉檔 40 秒～5 分鐘）。加上自動重試的查詢死結，**上線以來沒有任何一筆重試成功過**。判斷法：自動重試成功的紀錄「處理錯誤」會留著「第N次嘗試進行中」，2026-09 所有草稿裡一筆都沒有。
...
**錄音** `parseFilename()` —— ⭐ **抓得到日期就算可解析**，其餘盡力而為：

| 欄位 | 規則 |
|---|---|
| 日期（必要）| 開頭的 `YYYY-M-D`，容忍沒補零（`2021-10-2`）、前面包 `[ ]`（錄音筆檔名 `[2023-05-02][20-23-56]`）|
| 講員 | ① 結尾 `(講員)` → ② 結尾 `[_-空白]姓名+職稱`（`定睛望天-林信得弟兄`）→ ③ `(講員)` 後接短尾巴（`(張超雄傳道)上午`）→ ④ 都沒有就空白 |
| 主題 | 扣掉日期與講員的剩餘部分；空白時用聚會類型代替（`2020-08-07.mp3` → 「週五晚間」）|
| 時間 | 檔名內嵌的錄音時間 > 檔名寫的「上午／下午」> 週六看上傳時間 > `TYPE_TIMES` 預設 |
| 聚會類型 | 一週七天都有：週一～五「週X晚間」（真實時間在中午前／傍晚前則為「週X上午／下午」）、週六「安息日上午／下午」、週日「週日聚會」|

> ⚠️ **聚會類型只是標籤，永遠不要拿它當過濾條件。** 2026-09 之前只有二三五六日有類型，週一／週四回 null → `parseable=false` → 網站不顯示、cron 不轉。結果 2020 年起每年的週四佈道會（13 場）和講員格式不符的錄音（113 個）一直沒出現在網站上。
>
> 類型仍然保留，因為它有三個真實用途：Notion「聚會類型」欄位、同一天內的排序與標籤、**提供給 Gemini 當背景資訊**（`【聚會資訊】聚會類型：安息日上午`，當初為了修 AI 把安息日講道誤判成其他聚會而加的）。
>
> 改動 `parseFilename` 前，用 Drive 全部真實檔名跑新舊版本比對（舊版保留為 `_legacyParseFilename_unused`）。2026-09 改版時：原本可解析的 1,265 個中 1,213 個完全不變、52 個是修正（講員中間空白、主題結尾多餘的 `_`、檔名寫上午卻被判下午）；原本不可解析的 126 個全部可解析。
...
| 資料 | 何時載入 | 範圍 |
|---|---|---|
| **L1** Notion 紀錄 `listMeetings({from,to})` | 切換月／週／年時 | **只載入目前檢視的月份**，記憶體快取 2 分鐘（`notionByMonth`）|
| 去重索引 `meetingsIndex()` | 開網站時一次 | 全部紀錄的 fileId（24 KB），Worker 端 KV 快取 |
| **L2** `listUnprocessed(year, month)` | 切換月／週／年時 | 按月，sessionStorage 快取 5 分鐘（`DriveCache`）|
| **L3** `listStudy()` | 開網站時一次 | **全部**，因為預查的日期是估的，不能按月查 |

三個 Drive 載入函式（`loadDriveForMonth / Week / Year`）都會順便呼叫 `loadNotionForView` 載入同範圍的 Notion，所以**任何切換檢視的路徑都自動涵蓋**。新增切換入口時，走這三個函式之一就好。

> **2026-09 之前**是開網站就讀完全部 Notion（當時 546 筆、575 KB、3～5 秒），而且筆數越多越慢；「處理中」輪詢和 meeting 頁輪詢也每 10～15 秒讀一次全部。改成按月後單月約 12 KB、0.7 秒。
>
> ⚠️ **L2 / L3 去重不能只看已載入的月份** —— 預查的推估日期（createdTime）常跟 Notion 的真實日期不同月（例如 101 份推估在 2026-04-08、實際在 2023 年）；錄音若有人在 Notion 改過日期也會跨月。所以另外載入全部 fileId 的**索引**，去重集合 = 索引 ∪ 已載入月份。**索引到之前 `attachEntries` 不顯示任何待辦**，避免閃出一堆假的「待轉錄／待處理」。
>
> 索引的 KV 快取在 Worker 的 `notionFetch` 偵測到「新增紀錄」或「封存紀錄」時自動清除；直接在 Notion 手動改的東西最多 10 分鐘後反映。
...
Worker 的 CORS（`ALLOWED_ORIGIN` secret）只允許 `https://tjc-km.github.io`，從 `localhost` 開網頁直接呼叫 API 會被瀏覽器擋下 —— **這是正確的設定，不要為了測試放寬它**。

本機測試請用一個小代理：提供靜態檔，並把 `api.js` 裡的 `API_URL` 即時換成本機代理路徑、由代理去呼叫正式 Worker（伺服器端呼叫沒有 CORS 限制）。**代理只轉 GET**，避免測試時誤觸轉檔、封存等寫入動作。
...
> 📍 **位置**：Google 試算表「Line發送功能(小幫手)」（ID 見 `wrangler.toml` 的 `CONFIG_SHEET_ID`）的 **`Config` 分頁**，Worker 讀 `Config!A:B`。這個試算表**與 LINE 小幫手共用**，只動 `Config` 分頁、不要刪第一列標題。
...
| `AUTO_SKIP_KEYWORDS` | 非講道錄音關鍵字（逗號分隔），檔名含這些字的錄音**只顯示、排程不自動轉檔**；手動按「加入轉錄排隊」仍會轉。試算表沒有這一列時的預設值：`婚禮,喪禮,告別式,詩班`（敬老會、聖餐禮、洗腳禮、信徒會議有證道內容，要轉）。注意是「檔名包含」：`詩班教我的事(蕭正三弟兄)` 這種講道也會被擋 |
| `max_auto_size_mb` | 超過就不自動轉（只顯示），預設 150。手動排隊會直接標「失敗」（§3.1）|
| `min_auto_size_mb` | 小於就不自動轉（只顯示），預設 1 |
...
| 路徑 | 用途 |
|---|---|
| `GET /meetings?from=&to=` | 列出區間內的聚會（前端依檢視範圍呼叫）；不帶參數 = 全部（cron 用，前端不要用）|
| `GET /meetings/index` | 全部紀錄的錄音／預查 fileId（前端去重用，KV 快取 10 分鐘，新增／封存時自動清除）|
| `GET /meetings/:id` | 單一聚會 + 內文 blocks（自動分頁）|
| `GET /drive/list?year=&month=` | 某月 Drive 錄音 |
| `POST /drive/queue?fileId=` | 加入 KV 排隊（**正常手動流程**）|
| `POST /drive/process?fileId=` | 即時處理（前端已不再使用：fetch 的 `waitUntil` 約 30 秒就被砍，轉檔要 40 秒～5 分鐘）|
...
| 限制 | 影響 | 因應 |
|---|---|---|
| **50 subrequest / invocation** | 每份錄音約用 25–30 個；cron 前置作業約 15 個（卡死偵測 1、重試查詢 2、Drive 列檔 2 頁、`listMeetings` 每 100 筆 1 個、Config 1…）→ 第 2 份必爆 | `QUOTA_PER_RUN=1`，一次 cron 只做一份 |
...
一週七天都已經有預設類型，**通常不用改程式**：直接在 Notion 手動改該筆的「聚會類型」即可（前端顯示的是 Notion 的值，不是 worker 算的值）。要改預設時間才動 `TYPE_TIMES`。
...
| worker 單檔 3,300 行 | 可拆模組，但沒有拆的急迫性 | 低 |
...
| **cron 去重讀全部 Notion，subrequest 會越吃越多** | cron 的錄音去重用 `listMeetings` 讀全部（每 100 筆 1 個 subrequest，上限 `LIST_MEETINGS_MAX_PAGES=20` = 2000 筆）。2026-09 是 660 筆 / 7 個。估算 cron 最壞情況（大檔三段式）約 42 個，free tier 上限 50，**大約 1,400～1,500 筆時會撞到** —— 2026-09 起 cron 看得到約 940 份舊錄音，照每天 2～4 份的速度大概 2027 上半年會到。撞到的症狀是處理到一半丟 `Too many subrequests` → 卡死 → 重試也一樣失敗。解法：cron 去重改成 KV 存已處理 fileId 集合，或只讀 `錄音檔連結` 有值的頁 | **中**（有期限）|
| `app.js` 有死碼 | `handleProcess()` 已無呼叫端（手動流程改走排隊）| 低 |
| 掃描型 PDF 無法抽文字 | Drive OCR 不一定觸發 | 低（本質限制）|
| 檔名沒有日期的錄音完全看不到 | 無法放上行事曆；`/drive/list` 也是依檔名裡的 `YYYY-MM-` 查詢，所以連清單都沒有 | 低（本質限制，請撰寫者在檔名開頭加日期）|
...
4. **⚠️ 任何「取上限 N 筆」的迴圈都要能喊出來。** 2026-08 踩過一次：`listMeetings` 寫死跑 5 頁 = 500 筆，資料庫超過後**無聲截斷**。因為查詢依日期降冪，被砍掉的是最舊的紀錄 —— 整批 2021~2022 的資料寫進 Notion 後在網站上完全看不到，一度誤判成「寫入失敗」（實際 `/pages/:id` 查得到，只是列表讀不出來）。

   更危險的是 `runDailyProcess` 也用 `listMeetings` 建錄音 dedup 集合，截斷會讓舊錄音被當成「未處理」而**重新轉檔**，燒 Gemini 額度又製造重複紀錄。

   > **判斷法**：回傳筆數剛好是 100 的倍數（500、1000…）就要懷疑。

   2026-09 又踩一次：cron 列 Drive 音檔 `pageSize=500` 不翻頁，Drive 有 1,396 個，約 900 份舊錄音 cron 永遠看不到（§3.1）。

5. **「失敗只寫在 log」等於沒失敗。** 重試查詢被 Notion 回 400 整整兩個多月，只留下一行 `console.warn`，cron 其他部分照跑，所以沒人發現（§3.2）。檢查系統健康時，看 Notion 裡「待重試」的筆數跟最後編輯時間 —— 最後編輯時間很久沒動就代表重試沒在跑。
   > **寫法**：跑到 `has_more=false`，上限常數只當防無窮迴圈的保險，**逼近時 `console.warn`**。現有的 `LIST_MEETINGS_MAX_PAGES`、`getMeeting` 的 blocks 分頁、`STUDY_AUTO_MAX_CHECKS`、`QUOTA_PER_RUN` 都屬於這類，改動時一併檢視。
