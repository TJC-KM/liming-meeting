...
### ⚠️ 為什麼前端一定要帶 `requireContentDate=1`

`GET /study/list` **不讀內容**，`estimatedDate` 一律是 `getNextWednesdayTaipei(createdTime)`（[`handleStudyList`](worker/src/index.js)）。而多數舊檔是**同一天整批搬進 Drive** 的，createdTime 全部相同 —— 截至 2026-08 有 **131 份待處理預查的 estimatedDate 全是 `2026-04-08`**。

改成 fileId 去重後，原本的日期守衛不再擋這些檔案。若不帶 `requireContentDate=1`，這 131 份會全部堆到同一個週三。

因此 `api.js` 的 `processStudy()` **預設帶 `requireContentDate=1`**，日期只能靠 createdTime 時就拒絕寫入，前端再 `prompt()` 讓使用者手動指定日期（走 `date=` 覆寫，該參數會跳過此檢查）。**改動 `processStudy()` 時務必保留這個預設。**
