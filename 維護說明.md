...
| **預查（worker）** | **日期 + 聚會類型** | `isNotionAlreadyProcessed(env, dateStr, type)` |
| 前端 L2 / L3 | fileId（比對 `audioUrl` / `studyUrl`）| `attachEntries()` |

> ⚠️ **預查用「日期+類型」去重的後果**：同一個週三只能有一筆預查紀錄。若某份預查文件的內文日期打錯、撞到已存在的日期，就會被 [`index.js` 的 `isNotionAlreadyProcessed`](worker/src/index.js) 擋掉，回 `Notion 已有 YYYY-MM-DD 週三晚間 的紀錄`。
>
> 而且**預查失敗不會建 placeholder**（跟錄音不同），Notion 完全不留痕跡 → 使用者看到的現象是「按了沒反應、也沒有錯誤訊息」，極難自行診斷。
>
> 診斷方式：`GET /study/list` 看該檔的 `estimatedDate`，再比對 `GET /meetings` 該日期有沒有既存的週三晚間紀錄。
>
> 解法：確認正確日期後用 `date=` 覆寫（`POST /study/process?fileId=X&date=YYYY-MM-DD`），或確定舊紀錄該被取代時用 `overwrite=1`。
