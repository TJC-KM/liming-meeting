...
| `AUTO_SKIP_KEYWORDS` | 非講道錄音關鍵字（逗號分隔），檔名含這些字的錄音**只顯示、排程不自動轉檔**；手動按「加入轉錄排隊」仍會轉。試算表沒有這一列時的預設值：`婚禮,喪禮,告別式,詩班`（敬老會、聖餐禮、洗腳禮、信徒會議有證道內容，要轉）|
...
| 讀取「全部」Notion 的地方仍有 2000 筆上限 | 前端已改成按月載入，但 cron 的錄音去重與 `/meetings/index` 仍讀全部（`LIST_MEETINGS_MAX_PAGES=20`，每頁 1 subrequest）。目前 660 筆。逼近 2000 筆時 `runDailyProcess` 的 subrequest 預算會被吃掉（錄音處理本身要 25~30，free tier 上限 50），屆時 cron 去重要改成逐檔查詢或用 filter | 低（以目前速度還有數年）|
